...
- 字段说明:
  ...
  - `password`(string, 必填): 6-100 个字符，需包含字母与数字（如 `Abc123`）
- 成功响应(200):
```json
{
  "token": "<jwt>",
...
- 规则:
  - `newUsername`: 3-50 个字符，只能包含字母、数字、下划线，需唯一
...
- 接口: `POST /api/records/batch-delete`
- 描述: 批量删除当前用户的多条记录
- 请求头: `Authorization: Bearer <token>`, `Content-Type: application/json`
- 请求体:
```json
...
```
- 成功响应(204): 无响应体
- 失败响应: 400 请求参数错误；403 无权删除；404 部分记录不存在
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| phone | string | 否 | 11位手机号，修改时需唯一且匹配 `^1[3-9]\d{9}$` |
| avatar | string | 否 | 头像(支持 Base64 或 URL) |
